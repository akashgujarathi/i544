...
function updateAll(app, type){
  return async function(req, res){
  const result = getNonEmptyValues(req.body);
  console.log(Object.keys(result))
  console.log(result)
  let errors = validate(result, ['id']);
  console.log(!errors);
    ...
    try
    {
      ...
      if(type=== 'sensors'){
        ...
        res.redirect(type+".html?"+'&id='+result.id+'&modelNumber=&manufacturer=&quantity=&submit=search');
      }
    }
    catch(err){
      console.log(err)
